...
**Codebase:** Hogwarts Battle - A Harry Potter themed deck-building card game
**Type:** Local single-player terminal game (curses-based UI)
**Language:** Python 3.9+ (pure Python, no web frameworks)
**Architecture:** Object-oriented with plugin pattern for cards/villains
**Threat Model:** Minimal security concerns - this is a local game running with user's own permissions

**Key Patterns in This Codebase:**
- **Registry pattern:** Cards/villains self-register in `*_BY_NAME` dictionaries
- **Base class template methods:** `_effect()`, `_reward()`, `_on_reveal()` hooks
- **Game state threading:** Most methods take `game` parameter for state access
- **Display layer:** Curses windows/pads with careful refresh management
- **Configuration-driven:** YAML files define game setups

**Code Style Notes:**
- No type hints (older codebase - don't flag this)
- Mix of f-strings and `.format()` (don't flag this)
- Leading underscore means "override in subclass" OR "internal" (context-dependent)
- PascalCase classes, snake_case functions, SCREAMING_SNAKE_CASE constants

**Focus Areas for This Project:**
1. Game logic correctness (state management, card interactions)
2. Plugin pattern adherence (registration, base class implementation)
3. Curses display bugs (window sizing, unicode rendering)
4. Edge cases in game rules
5. Configuration validation (YAML references to existing classes)

**De-prioritize for This Project:**
- Traditional web security (no network, no untrusted input)
- Style modernization (respect existing conventions)
- Architectural refactoring (respect existing patterns)
